...
describe('RLP encoding (integer):', function () {
  ...
  it('it should handle zero', function () {
    assert.strictEqual(RLP.encode(0).toString('hex'), '80')
  })
})
...
describe('typed lists:', function () {
  ...
  it('encode a nested list', function () {
    const valueEncoded = RLP.encode(valueList)
    // prettier-ignore
    assert.deepStrictEqual(valueEncoded, new Buffer([0xd2, 0xc3, 0x01, 0x02, 0x03, 0xcd, 0x83, 0x04, 0x05, 0x06, 0x83, 0x07, 0x08, 0x09, 0xc4, 0x00, 0x82, 0xab, 0xcd]))
  })
})
...
describe('null values', function () {
  ...
  it('should decode a null value', function () {
    assert.deepStrictEqual(Buffer.from([]), RLP.decode(Buffer.from('80', 'hex')))
  })
})
